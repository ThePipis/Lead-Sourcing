  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-slate-900 border border-slate-800 p-5 rounded-xl flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div>
          <div className="flex items-center space-x-2 text-xs text-amber-400 font-mono mb-1 font-bold">
            ...
            <span>FULL-STACK ENTERPRISE SYSTEM ARCHITECTURE</span>
          </div>
          <h2 className="text-xl font-bold text-white tracking-tight">
            FastAPI Backend + React Frontend + SpatiaLite / PostGIS
          </h2>
          <p className="text-xs text-slate-400 mt-1 max-w-3xl">
            Arquitectura desacoplada de alto rendimiento. Diseñada para procesar matrices vectoriales con NumPy en milisegundos, 
            orquestar APIs externas (Yelp, Geoapify, Data Axle, Llama.cpp) y exportar manifiestos conformes con las especificaciones técnicas postales.
          </p>
        </div>

        <div className="flex items-center space-x-2 text-xs font-mono bg-slate-950 px-3 py-2 rounded-lg border border-slate-800 text-slate-300">
          <Server className="w-4 h-4 text-emerald-400" />
          <span>FastAPI 0.115 + Pydantic v2 + SQLAlchemy 2.0</span>
        </div>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 space-y-3">
        <div className="flex items-center justify-between border-b border-slate-800 pb-2">
          <div className="flex items-center space-x-2 text-xs font-bold text-white uppercase tracking-wider">
            ...
            <span>1. Estructura Completa de Directorios y Archivos</span>
          </div>
          <button
            onClick={() => handleCopy('tree', DIRECTORY_TREE_TXT)}
            className="flex items-center space-x-1 text-xs text-slate-400 hover:text-white bg-slate-950 px-2 py-1 rounded border border-slate-800"
          >
            {copiedKey === 'tree' ? <Check className="w-3 h-3 text-emerald-400" /> : <Copy className="w-3 h-3" />}
            <span>Copiar Árbol</span>
          </button>
        </div>
        <pre className="bg-slate-950 p-4 rounded-lg border border-slate-800 font-mono text-xs text-slate-300 overflow-x-auto leading-relaxed">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl overflow-hidden space-y-0">
        <div className="flex flex-wrap items-center justify-between bg-slate-950 border-b border-slate-800 p-3 gap-2">
          <div className="flex items-center space-x-2">
            <FileCode className="w-4 h-4 text-amber-400" />
            <span className="text-xs font-bold text-white uppercase tracking-wider">
              2. Código de Producción Backend (FastAPI / Python)
            </span>
          </div>
          ...
          <div className="flex items-center space-x-2">
            <button
              ...
              className={`px-3 py-1 rounded text-xs font-semibold font-mono ${
                selectedSnippetKey === 'models' ? 'bg-amber-500 text-slate-950' : 'text-slate-400 hover:text-white'
              }`}
            >
              models.py
            </button>
            <button
              ...
              className={`px-3 py-1 rounded text-xs font-semibold font-mono ${
                selectedSnippetKey === 'propensity' ? 'bg-amber-500 text-slate-950' : 'text-slate-400 hover:text-white'
              }`}
            >
            ...
            <button
              ...
              className={`px-3 py-1 rounded text-xs font-semibold font-mono ${
                selectedSnippetKey === 'lead_service' ? 'bg-amber-500 text-slate-950' : 'text-slate-400 hover:text-white'
              }`}
            >
            ...
            <button
              ...
              className={`px-3 py-1 rounded text-xs font-semibold font-mono ${
                selectedSnippetKey === 'export_service' ? 'bg-amber-500 text-slate-950' : 'text-slate-400 hover:text-white'
              }`}
            >
        ...
        <div className="p-4 bg-slate-950">
          <div className="flex items-center justify-between text-xs font-mono text-slate-400 pb-2 border-b border-slate-800/80 mb-3">
            <span className="text-amber-400">{activeSnippet.filename}</span>
            <button
              onClick={() => handleCopy(selectedSnippetKey, activeSnippet.code)}
              className="flex items-center space-x-1 bg-slate-900 hover:bg-slate-800 text-slate-300 px-2.5 py-1 rounded border border-slate-800 text-[11px]"
            >
          ...
          <pre className="p-2 font-mono text-xs text-slate-300 overflow-x-auto leading-relaxed max-h-[500px]">
            {activeSnippet.code}
          </pre>
        </div>
      </div>
